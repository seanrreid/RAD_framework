...
- Identify the post-check invocation pattern: the scripts (check-tests.sh, check-scope.sh, lint-plan.sh, check-role.sh, open-pr.sh) are bash-wrapped CLI tools that exit 0/1/2 and print to stdout
...
- Flag duplication against hard-coded test-veto in check-tests.sh (exit 1 for missing tests) so hook-runner need not re-implement test file presence checks

## Scope

Exact read scope: `harness/adapters/git-state-store.js`, `harness/adapters/agent/contract.js`, `scripts/**` (esp. check-tests.sh, check-scope.sh, lint-plan.sh, check-role.sh, open-pr.sh). Nothing else.
...
4. Dedup notes: "check-tests.sh already gates test presence at exit 1; lint-plan.sh validates plan structure; check-scope.sh validates file-change scope; check-role.sh validates architect identity"
