...
- The design doc lives on `rad/harness-audit` and is not yet on the default branch; the work branch is cut from default, so executors must reference the spec from the audit branch until it merges. (Resolved during delivery: the design docs were merged to `main` via PR #11.)
- Framework gap (surfaced during this cycle's `/rad-review`): `scripts/check-scope.sh` exempts the plan doc and execution log but **not** `.agents/findings.jsonl`, so the review log this cycle legitimately appends counts as a scope violation. Worked around by declaring `findings.jsonl` in Files in Scope; the proper fix is to exempt RAD bookkeeping files (`findings.jsonl`, `.agents/logs/`) in `check-scope.sh` — a candidate for a follow-up framework plan.
