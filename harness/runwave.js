 ...
 * @returns {string}
 */
function buildWavePrompt(wave, planCtx) {
  const {
    feature,
    branch,
    executionLog,
 ...
 */
function extractWaveResultBlock(text) {
 ...
 */
function parseWaveResult(block) {
...
/**
 * Build a synthetic failure result when the SDK call itself fails.
 *
 ...
 * @returns {{ status: string, tasks: Array }}
 */
function syntheticFailure(waveId, errorMessage) {
  return {
    status: 'failed',
    tasks: [
      {
